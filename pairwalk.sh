...
DEFAULT_MODEL="openrouter/z-ai/glm-5.3-flash:high"
...
or clean up: bash $0 release"
  ...
  local follow_clause
  if [ -n "$orch" ]; then
    follow_clause="FOLLOW-UPS: a message starting with FOLLOWUP means the orchestrator updated the plan — todo init the referenced new items and execute them under this same contract (validation, commit seams, escalation ladder). If the user sends a tweak directly to you that is not a FOLLOWUP, do not improvise: run herdr agent prompt $orch \"FOLLOWUP REQUEST: <tweak>\" and wait for the orchestrator to plan it."
  else
    follow_clause="FOLLOW-UPS: a message starting with FOLLOWUP means the plan was updated — todo init the referenced new items and execute them under this same contract. There is no orchestrator channel: if a user tweak is ambiguous, ask in your report instead of improvising."
  fi

  local prompt
  prompt="$(printf 'You are pairwalk executor agent %s, bound to this worktree: only act on prompts addressed to your name. Execute the plan in %s (repo root = current directory). First run todo init from its numbered items and treat that list as the implementation contract. Do not re-plan or re-architect; adapt small implementation details only, and if a core assumption is false, stop and report the mismatch instead of inventing a new design. Work along the plan%s natural commit seams and run each item%s stated validation. Never commit or stage %s. The orchestrator pane stays read-only while you execute: you own all edits in this worktree. %s %s' \
    "$name" "$PLAN_FILE" "'" "'" "$PLAN_FILE" "$esc_clause" "$follow_clause")"
...
cmd_dialog() {
  ...
  local kickoff
  kickoff="$(printf 'You are pairwalk orchestrator agent %s for the fresh worktree at %s (branch %s, repo %s). The NEXT message I send is the actual task; until then only acknowledge with one short line. On the task: (1) investigate the repo READ-ONLY; (2) write PLAN.md at the worktree root — numbered, dependency-ordered items, each with outcome, area, validation, and natural commit seam, plus assumptions and acceptance criteria; PLAN.md is your ONLY write during planning; (3) run: bash %s/pairwalk.sh spawn — that starts the OMP executor split on the runner model with the plan as its contract; (4) after spawn, resolve YOUR runner and use only that name for all executor commands: RN=$(bash %s/pairwalk.sh runner-for) — never address a runner by guessing from herdr agent list, and never reuse a runner name from memory; monitor with plain settled waits: herdr agent wait $RN (returns when the runner is idle, done, or blocked — never use --until blocked alone, a finished runner never becomes blocked), herdr agent read $RN, review with git diff; corrections via herdr agent prompt $RN "<fix>" --wait; (5) while the executor runs you stay read-only in this worktree — never edit files yourself; (6) when the executor finishes, verify the cumulative diff against the PLAN.md acceptance criteria, report DONE, and STAY AVAILABLE — the session continues; (7) ESCALATION LADDER: a runner message starting with "ESCALATE <item>" means that item failed twice on the runner and is now yours. You own exactly that item: implement it yourself in the worktree — this temporarily lifts your read-only restriction for that item only — satisfy its stated validation, and if the plan marks it [gate], apply the gating mechanism it references. Before replying, verify the target: herdr agent get <name-from-message> must report cwd inside YOUR worktree; if it does not, do not send — re-resolve with runner-for. When verified, reply: herdr agent prompt <runner-name> "ESCALATION DONE <item>: <what you did>", then return to read-only monitoring. (8) FOLLOW-UPS: this workflow loops. When I send a follow-up tweak or question after the first report: investigate read-only, then amend PLAN.md — append new numbered items under a "## Follow-ups" section (or amend existing ones), keeping PLAN.md your only write — and hand execution back to the runner: re-resolve RN=$(bash %s/pairwalk.sh runner-for); if it resolves and is alive, run herdr agent prompt $RN "FOLLOWUP: PLAN.md updated — todo init the follow-up items and execute under the same contract" and monitor with plain settled waits (herdr agent wait $RN); if it does not resolve (executor released), run spawn again so the updated PLAN.md is picked up. Monitor, verify, and report exactly as in (4)-(6); the escalation ladder applies unchanged to follow-up items. Never commit or stage PLAN.md.' \
    "$pname" "$wt" "$BRANCH" "$repo" "$SCRIPT_DIR" "$SCRIPT_DIR" "$SCRIPT_DIR")"
